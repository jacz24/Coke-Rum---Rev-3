...
class Table(object):
    ...
    def __init__(self, seats = 8, quiet = False, training = False):
        ...
        self.teacher = xmlrpc.client.ServerProxy('http://localhost:8080')
    ...
    def start_hand(self, players):
        ...
        assert sum([p.stack for p in players]) == 4000*len(self._seats)
        self.new_round()
        self._round=0
        ...
        while self._round<4 and len(players)>1:
            ...
            assert sum([p.stack for p in self._seats]) + self._totalpot == 4000*len(self._seats)
