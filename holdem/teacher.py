...
class Teacher(Thread):
    ...
    #LLOOOOOOOOKKKKKK HHHEEEERRRRREEEEE FOOOOOOORRRR THHHHeeee
    ...
    def reset_game(self):
        for p in self.players:
            if p.get_ai_id() not in [-1, None, 1,2,3,4]:
                p.rejoin_new(self.test_pool.pop())
            else:
                p.rejoin()
    def add_human(self):
    # add human
        self.players.append(PlayerControlProxy(PlayerControl('localhost', 8000+1, 1, False, None)))
    
    def add_allin(self):
    # all-in
        self.players.append(PlayerControlProxy(PlayerControl('localhost', 8000+1, 1, True, 3)))
    def add_randombot(self):
    # random bot
        self.players.append(PlayerControlProxy(PlayerControl('localhost', 8000+2, 2, True, 1)))
    def add_randombotsecond(self):
    # random bot
        self.players.append(PlayerControlProxy(PlayerControl('localhost', 8000+3, 3, True, 1)))
    def add_checkcallbot(self):
        self.players.append(PlayerControlProxy(PlayerControl('localhost', 8000+4, 4, True, 2)))
    def add_nncontrollers(self):
    # nn init
        for i in range(5,self.seats+1):
...
class TeacherProxy(object):
    def __init__(self, teacher):
        ...
        self.server = SimpleXMLRPCServer(('localhost', 8080), logRequests=False, allow_none=True)
